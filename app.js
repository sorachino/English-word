...
function updateVoiceCloudNote() {
  ...
  if (ttsOverThreshold()) {
    el.textContent = `今月の無料枠の85%に達したため、一時的にSamantha（標準の読み上げ）に切り替わっています。今月の無料枠の使用量 ${pct}%`;
  } else {
    el.textContent = `Google Cloud Text-to-Speech（Neural2）を使用中です。今月の無料枠の使用量 ${pct}%`;
  }
}
